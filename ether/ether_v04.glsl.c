...
{
    return sin(20 * p.x * sin(fGlobalTime * 0.01)) * sin(bass() * 0.1 * sin(fGlobalTime * 0.01) * 20 * p.y);//*sin(20*p.z);
}
...
vec3 render(vec2 uv, float t, int iters)
{
    ...
    for (int i = 0; i <= ITERS; ++i)
    {
        ...
        float dist = mix(min(map2(p), map3(p)), map(p), 3 - length(uv));
        float f = clamp((dist - map2(p + .1)) * 0.5, -0.1, 1.0);
...
vec3 render_iter(vec2 uv, float t)
{
    ...
    for (float i = 0.; i < iters; i++) {
        ...
        acc.g += render(uv + vec2(0.05, -0.05), t + 2. * i * dt, 7).g;
...
void main(void)
{
    ...
    vec3 col;
    col = render(uv, fGlobalTime, 1);

    //TODO: float bpm = 120.f;

    float h = abs(sin(fGlobalTime * 0.2) * 0.4);
    col = smoothstep(col, vec3(1) - col, vec3(h));
