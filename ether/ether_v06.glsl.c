...
{
    return 0.0;//sin(20*p.x*sin(fGlobalTime))*sin(bass()*0.1*sin(fGlobalTime*0.)*20*p.y);//*sin(20*p.z);
}
...
{
    vec3 col = vec3(0);
    float d = 4.0;//* abs(sin(bass())); // Camera distance
    ...
    for (int i = 0; i <= ITERS; ++i)
    {
        ...
        float f = clamp((dist - map3(p + .1)) * 0.5, -0.1, 1.0);
...
vec3 render_iter(vec2 uv, float t)
{
    ...
    for (float i = 0.; i < iters; i++) {
        ...
        acc.r += render(uv + vec2(-0.15, 0.1), t + i * dt, 2).r;
        acc.g += render(uv + vec2(0.15, -0.05), t + 2. * i * dt, 7).g;
        acc.b += render(uv + vec2(-0.12, -0.1), t + 3. * i * dt, 3).b;
    }
    return acc / iters;
}
